...
module.exports = {
  ...
  /**
   * Return a promise that rejects with a SpaceTradersError.
   * @param {String} message the error message
   * @param {Object} options the error options
   */
  reject: function(message, options) {
    return Promise.reject(new SpaceTradersError(message, options));
  },
  /**
  ...
  explainStdin: function() {
    if (isWindows) {
      throw new SpaceTradersError('STDIN input is not available on Windows.', {exit: 1});
    }
