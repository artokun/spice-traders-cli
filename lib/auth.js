...
var INVALID_CREDENTIAL_ERROR = new SpaceTradersError(
  'Authentication Error: Your credentials are no longer valid. Please run ' + chalk.bold('spacetraders login --reauth') + '\n',
  {exit: 1}
);
...
var _getPort = denodeify(portfinder.getPort);
...
var logout = function(refreshToken) {
  ...
  return api.request('GET', '/o/oauth2/revoke', {
    ...
    throw new SpaceTradersError('Authentication Error.', {
      exit: 1
    });
  });
};
