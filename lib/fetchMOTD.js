  ...
  if (motd && motdFetched > Date.now() - ONE_DAY_MS) {
    if (motd.minVersion && semver.gt(motd.minVersion, pkg.version)) {
      logger.error(
        ...
        chalk.bold('npm install -g spacetraders-cli'),
        'to upgrade.'
      );
      process.exit(1);
    }
    ...
    // axios.get({
    //   url: utils.addSubdomain(api.realtimeOrigin, 'spacetraders-public') + '/cli.json',
    //   json: true
    ...
    // TODO: set up MOTD in public server
    latestVersion('spacetraders-cli').then(version => {
      motd = {
        fetched: 0,
        minVersion: version,
        message: 'Welcome to SpaceTraders MMO.\nGame is under active development'
      }
      configstore.set('motd', motd);
      configstore.set('motd.fetched', Date.now());
    })
