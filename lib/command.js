  ...
  cmd.action(function() {
    ...
    if (arguments.length - 1 > argCount) {
      return client.errorOut(
        new SpaceTradersError('Too many arguments. Run ' + chalk.bold('spacetraders help ' + cmd._name) + ' for usage instructions', {exit: 1})
      );
    }
    ...
    runner.apply(self, arguments).then(function(result) {
      ...
      var preppedMessage = chalk.stripColor(err.message || '').replace('"command.register, needs unique user identifier"', '<namespace>');
